...
- **Today's execution plan written** (`S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md`): sequences the full day's build — Layer 0 (generic foundation: Category+cutover flag, Blueprint Catalog schema, Knowledge Library schema, Project Schedule facet, Skill/Routine catalog + Dispatcher path, Select-Entities settings screen) → Layer 1 (populate Blueprint + Knowledge Library, wire live Anthropic API calls into the Dispatcher, activate the 4 chosen projects, run the cutover push+lockout) → Layer 2 (product-line dashboard, end-to-end surface-blend pass). Q2 Loan Reconciliation runs as a fully separate, parallel track (not a Supabase build task — Credit Desk is Phase 2).
- **Scheduling scope resolved** (`MS_Project_Style_Scheduling_Engine_Scope.md`, from a parallel chat): one `tasks` table for checklist items + schedule items alike (scheduling columns nullable) — `duration`/`start_date`/`end_date`/`scheduling_mode`/`constraint_type`/`constraint_date`; a `task_dependencies` table (FS/SS/FF/SF + lag); parent-child cross-project visibility via a linked, auto-recalculated milestone task (Option B, not a shared row). **Today's Layer 0 builds only this v1 scope.** The full MS Project-style engine (critical path calc, resource assignments, baselines, interactive drag-cascade — that doc's §5) is explicitly flagged as its own future build-doc phase, not part of today. "AI Integration #3" was a stray autonumber — no third item existed. **Only remaining open input: the 4 project names**, needed before Layer 1 activation.

---

## Next Steps (in order)

1. [ ] **ACTIVE TODAY — execute `S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md`** in the Claude Code session. Layer 0 first (generic foundation, incl. v1-scope scheduling per `MS_Project_Style_Scheduling_Engine_Scope.md` §1–4 only), then Layer 1 (populate/activate), then Layer 2 (surface). Get the 4 project names from Clint before Layer 1 activation. Q2 Loan Reconciliation runs in parallel on existing tools, independently of this sequencing.
...
7. [ ] **Future phase, explicitly not today — full MS Project-style scheduling engine.** Per `MS_Project_Style_Scheduling_Engine_Scope.md` §5: critical path calculation, cascading recalculation, `task_resource_assignments` + `schedule_baselines` tables, constraint-type enforcement, interactive drag-to-reschedule. Scope once v1 (today's Layer 0 build) is live and proven.
...
- **Today's plan open inputs** — see `S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md` bottom section: the 4 chosen project names, and which tool generates Q2 loan invoices. (Scheduling scope and "AI Integration #3" are resolved.)
