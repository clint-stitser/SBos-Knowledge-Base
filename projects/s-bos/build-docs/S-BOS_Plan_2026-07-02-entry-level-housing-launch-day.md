...
## NEW DECISION — 2026-07-02: Universal Notes & Comments + Polymorphic Task Linking

**Notes & Comments generalizes from its current scope (Projects/People/Companies, live) to a universal per-type pattern** — same shape as the versioning decision above, applied to discussion instead of document history. Rationale: notes/comments are how organizational learning, discussion, and alignment get woven into the system; restricting them to the three original drivers leaves Budget items, Schedule/Tasks, Project participants/stakeholder links, Documents, KB entries, and Skills without a discussion thread.

- **Pattern: polymorphic `parent_type` + `parent_id` link** (not per-type sibling tables like versioning — comments need to attach to a growing, not-fully-known-yet universe of object types, so the link stays flexible rather than requiring a new table per type).
- **Referential integrity via a `parent_types` registry table**, not a DB-enforced foreign key (Postgres can't FK a polymorphic column) and not a rigid CHECK constraint (would require an `ALTER TABLE` per new type). The registry holds one row per valid `parent_type` → its actual table; a trigger validates `parent_id` exists in that table before insert. **Adding a new commentable/taskable object later = one new registry row, not a migration.** This is the standard pattern for this problem (cf. Rails polymorphic associations) — not experimental, but requires the registry+trigger discipline to avoid orphan-record risk.
- **Scope today, not forever:** build polymorphic Comments + Task linking now, scoped initially to whatever Layer 0/1 actually touches (Projects, People, Companies, Budget items, Schedule/Tasks). Future objects (Documents, KB entries, Skills, anything else) get added by inserting a registry row — explicitly **not** a full-scope build today.

**Comment → Task upgrade (closes a real gap in the existing "one universal Task table" decision, PDD §3):**
- Today's system attaches a "follow up" flag to a note/comment, which is invisible to the Task table — meaning follow-ups don't show up in resource/people allocation analysis built on tasks. This defeats the point of having one universal Task table.
- **Fix:** the UI action becomes "create/attach a task" on a comment, not "flag as follow-up." The resulting Task row's `parent_type`/`parent_id` is set to **the object the comment is attached to** (e.g., the budget line, the schedule item) — not to the comment itself. The Task table therefore gets the same polymorphic `parent_type`/`parent_id` link as Comments, using the same `parent_types` registry.
- **Build impact on today's plan:** the Task table's parent-link shape is being built **today**, in Layer 0 item 4 (Schedule facet v1) — this decision needs to land in that build, not retrofit after. Flagged inline in Layer 0 below.
...
7. **Universal Comments table + `parent_types` registry** — per the Universal Notes & Comments decision above. Polymorphic `parent_type`/`parent_id`, registry + trigger for referential integrity, `follow_up_task_id` column pointing at the Task table (item 4). Scoped initially to Projects/People/Companies/Budget items/Schedule-Tasks — same scope as item 4's Task linking, since they share the registry. Natural to build alongside item 4 (shared registry table).

> **Note:** Project documents (legal docs, design docs, invoices) don't have a dedicated Layer 0 schema item in this plan yet — they fall under the Project Drive facet, not yet scheduled today. When that facet is built, `project_document_versions` applies per the versioning decision above, and the facet's records become a `parent_type` in the Comments/Task registry.
...
- [ ] **Expanding the Comments/Task `parent_types` registry beyond today's initial scope** (Documents, KB entries, Skills, Templates) — add via registry row when those facets are built, not a today task.
