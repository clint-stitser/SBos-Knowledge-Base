...
- **Proactive execution partner, not create-only or passive Q&A.** Every capability runs the same loop — **Observe → Identify → Propose → Execute**: read the full situation (structured record + scattered context), surface opportunities/risks/deficiencies, propose concrete fixes *with their source* and converse, then carry the approved ones out (confirming on consequential/irreversible actions). Canonical example = the **Credit Desk loan "Audit"** (scans terms + ledger/interest math + documents incl. text + compliance/next-due → finds deficiencies like a missing maturity or undocumented payment → proposes fixes with source ("set maturity 2028-12-31 — from the note") → applies approved fixes, files an audit-findings report, and opens assigned, due-dated tasks for the rest). Ecosystem-aware capture, auto-generated GYR reports, and mass-cleanup are the same pattern at other scopes. Full detail in `shared/operator-assistant.md`.
