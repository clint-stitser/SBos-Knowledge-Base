...
### Today's Execution Plan + Scheduling Scope — 2026-07-02
- **Full capture:** `S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md` — sequences the day as Layer 0 (generic foundation) → Layer 1 (populate/activate Entry-Level Housing) → Layer 2 (surface/dashboard), with Q2 Loan Reconciliation running as a fully separate parallel track (not a Supabase task — Credit Desk is Phase 2).
- **Scheduling scope resolved** (`MS_Project_Style_Scheduling_Engine_Scope.md`, from a parallel chat): **one `tasks` table** for checklist items and schedule items alike — a checklist item is just a task with scheduling columns left null (`duration`, `start_date`, `end_date`, `scheduling_mode` manual/auto, `constraint_type`, `constraint_date`). Predecessors/successors live in a separate `task_dependencies` table (FS/SS/FF/SF + `lag_days`), not columns on `tasks` — a task can have multiple predecessors. **Cross-project (parent-child) task visibility resolved as Option B (chosen default):** the child task stays owned/edited only at the child project; the parent gets its own visually-distinct milestone task, connected via the same `task_dependencies` table, `scheduling_mode=auto`, recalculated by a simple Postgres trigger when the child's date changes — no shared row, no permission gating needed. (Option A — a literal shared row via a `task_projects` junction table — stays available only for the rare genuinely-dual-owned case, not the default.)
- **Scope boundary — v1 vs. full engine:** today's Layer 0 build is **v1 only** (the schema above, sufficient for dependencies/durations/start-end dates). The **full MS Project-style engine** — critical path calculation, cascading recalculation, `task_resource_assignments` + `schedule_baselines` tables, constraint-type enforcement, interactive drag-to-reschedule — is explicitly scoped in that doc's §5 as its own future build-doc phase, not part of today. Don't conflate the two when building.

### Universal Notes & Comments + Polymorphic Task Linking — 2026-07-02
- **Notes & Comments generalizes** from its current live scope (Projects/People/Companies) to a **universal per-type pattern**, closing a gap flagged by Clint: Budget items, Schedule/Tasks, Project participants/stakeholder links, Documents, KB entries, and Skills had no discussion thread despite the Brain's premise that notes/comments weave organizational learning and alignment into the system.
- **Mechanism: polymorphic `parent_type`/`parent_id` link**, not per-type sibling tables (unlike the document-versioning pattern decided the same session, below) — comments need to attach to a growing, not-fully-known universe of object types, so the link stays flexible. Referential integrity comes from a **`parent_types` registry table** (one row per valid type → its actual table) + a trigger validating `parent_id` exists — not a DB-enforced FK (impossible on a polymorphic column) and not a CHECK constraint (would need an ALTER TABLE per new type). Adding a future commentable/taskable object = one registry row, not a migration. Standard pattern (cf. Rails polymorphic associations), not experimental.
- **Scope today, not forever:** built now for Projects/People/Companies/Budget items/Schedule-Tasks (Layer 0 item 7 of today's plan); Documents/KB entries/Skills/Templates get added via registry row when those facets are built — explicitly deferred, not a gap to fix later.
- **Comment → Task upgrade closes a real gap in the existing "one universal Task table" decision (PDD §3):** the live system's "follow up" flag on a note/comment is invisible to the Task table, so follow-ups don't show up in resource/people-allocation analysis — defeating the point of having one universal Task table. **Fix:** the UI action becomes "create/attach a task" on a comment; the resulting Task row's `parent_type`/`parent_id` points at **the object the comment is attached to** (e.g. the budget line), not the comment itself. The Task table gets the same polymorphic link + registry as Comments.
- **Build impact:** both the Comments table and the Task table's polymorphic link are being built **today**, in Layer 0 (items 4 and 7) of `S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md` — landing in the initial build, not a retrofit.

### System-Wide Document Versioning — 2026-07-02
- **Applies to** Knowledge Library entries (all levels), project documents (legal docs, invoices, etc.), the Document/Template Library, and AI-interface documents (skills/routines/prompts).
- **Pattern: per-type, append-only version tables** (not polymorphic — contrast with Comments/Tasks above): each parent table (`kb_entries`, `project_documents`, `templates`, `skills`) gets a sibling `_versions` table — `id, parent_id, version_number, created_by, created_at, commit_message` + `content` (text docs, diffable) or `storage_path` (file docs, supersede-and-keep). Parent carries `current_version_id`; rollback = repoint, never delete.
- **`supa_audit` runs underneath as a passive backstop** (trigger-based row change logging) — layered under the intentional versions tables, not a replacement.
- **Storage cost/retention policy for file versions explicitly deferred** until the system is running and usage patterns are understood.
- **Build impact:** Layer 0 schema items that store documents/content (Knowledge Library, Blueprint/Template Catalog, Skill/Routine catalog) include their `_versions` table from the start. Full detail in `S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md`.
...
- [ ] **Today's execution plan — active.** See `S-BOS_Plan_2026-07-02-entry-level-housing-launch-day.md`. Only remaining open input: the 4 project names.
- [ ] **Full MS Project-style scheduling engine — future phase, not today.** `MS_Project_Style_Scheduling_Engine_Scope.md` §5: critical path, cascading recalculation, resource assignments, baselines, interactive drag behavior. Scope once v1 is live and proven.
- [ ] **Expand the Comments/Task `parent_types` registry beyond today's initial scope** (Documents, KB entries, Skills, Templates) — add via registry row when those facets are built, not a today task.
- [ ] **Storage cost/retention policy for file-based document versions** — deferred until the system is running and usage patterns are understood.
