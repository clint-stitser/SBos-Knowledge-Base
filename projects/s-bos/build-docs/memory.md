...
- **Access (open):** Supabase RLS keyed off People/Companies linked to a record; Softr per-group UI perms → role flags + RLS + conditional UI. Confirm role→permission matrix (auth-plan.md).
