...
(function () {
  ...
  const PROXY = 'https://sb-planning-tools-production.up.railway.app';

  /* ── Editable element selector ──────────────────────────────── */
  // Targets text content inside the article body only.
  // Block-level only — avoids making nested inline spans editable twice.
  const EDITABLE_SEL = '#articleBody p, #articleBody h2, #articleBody h3, #articleBody li, #articleBody td, #articleBody th, #articleBody blockquote > p, #articleBody .callout-box > p, #articleBody .layer-detail > p, #articleBody .layer-detail > strong, #articleBody .pillar-card-name, #articleBody .pillar-card-desc, #articleBody cite';
  ...
  async function saveToGithub(editorName) {
    const filePath = (typeof LESSON_CONFIG !== 'undefined' && LESSON_CONFIG.filePath)
      ? LESSON_CONFIG.filePath : null;
    if (!filePath) throw new Error('LESSON_CONFIG.filePath is not defined for this article.');
  ...
  async function loadHistory() {
    ...
    if (!filePath) {
      list.innerHTML = '<p class="history-loading">No file path configured.</p>';
      return;
    }
    try {
